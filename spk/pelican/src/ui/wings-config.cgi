...
<body>
    ...
    <script>
        const CGI_URL = '/webman/3rdparty/pelican_panel/wings-config.cgi';
...
check_status() {
    ...
    container_state=$(docker inspect -f '{{.State.Running}}' pelican_panel-wings-1 2>/dev/null)
...
save_config() {
    ...
    docker compose -f "/var/packages/${PACKAGE}/target/share/docker/compose.yaml" \
        --env-file "/var/packages/${PACKAGE}/var/panel.env" \
        restart wings >/dev/null 2>&1 &
...
case "${QUERY_STRING}" in
    *action=status*)
        check_status
        ;;
    *action=get-config*|*action=get_config*)
        get_config
        ;;
    *action=save-config*|*action=save_config*)
        save_config
        ;;
    *)
        send_html
